...
class _Packet_logger:
    ...
    def __init__(self, channel_start_time, log_file):
        ...
        log_line.extend(["transmission type", "number of epr consumed", "number of transmissions", "sent"])
        self._write_log_line(log_line)

    def log_packet(self, sender, receiver, start_time, end_time, measurement_time, packet_bits,
                   epr_used, number_of_transmissions, transmission_type, dk):
        ...
        log_line = [sender, receiver, format(float(start_time.total_seconds()), ".2f"),
                    format(float(end_time.total_seconds()), ".2f"),
                    format(float(transmission_time.total_seconds()), ".2f")]
        ...
        log_line.extend([transmission_type, str(epr_used), str(number_of_transmissions), dk])
