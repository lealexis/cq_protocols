...
def packet_processing(pkt):
    ...
    with open(LOGFILE, "a") as logfile:
        ...
        packet = IP(pkt.get_payload()) # | header |  -------     | payload |
        packet_bits = to_bit_array(packet) # | payload |
        # from_bit_array(packet_bits) = packet = IP(PKT.GET_PAYLOAD())

# A ---> bridge a ~~~~~~~~ bridge b ----> B
# A pkt---->pkt-bridge a -packet_bits >~~~~> new_packet_bits - bridge b - new packet ---> B
        ...
        new_packet_bits = quantum_protocol.transmit_packet(packet_bits, source_address, "normal") # |payload |TerByte| = transmit_packet(| payload |)
        # |header| payload | ---> depolchann
        # |header| payload | = IP(some packet) ---- > new_pkt
        print("SENDING ONWARDS")
        # new packet = from_bit_array(new_packet_bits)
