...
class EPR_buff_itfc(object):
        ...
        if self.in_process and self._get_MSSG_in_process() == "EPR:wait_ACK/NACK":
            if to_history:
                ...
                if len(f)==0:
                    ...
                    if self.is_receiver:
                        actualize_df = pd.DataFrame([[F_est, int(1),t_fin, 0]], 
                                                columns=["F_est", "ACK" ,"t_finish", "t_end_sdc"])
                    else:
                        actualize_df = pd.DataFrame([[F_est, int(1),t_fin, 0]], 
                                                columns=["F_est", "ACK" ,"t_finish", "Fid"])
                    self._actualize_histories(df_to_add=actualize_df, kind="epr")
                    if len(self.u_IDs)==0:
                        self.is_empty = False
    ...
    def drop_ip_frame_EPR_END_PHASE(self, fest=None):
        if self.in_process and self._get_MSSG_in_process()=="EPR:wait_ACK/NACK":
            ...
            t_fin = time.perf_counter() - self.start_time
            if self.is_receiver:
                actualize_df = pd.DataFrame([[fest, int(0), t_fin, 0]], 
                                            columns=["F_est", "ACK", "t_finish", "t_end_sdc"])
            else:
                actualize_df = pd.DataFrame([[fest, int(0), t_fin, 0]], 
                                            columns=["F_est", "ACK", "t_finish", "Fid"])
    ...
    # *correct in process EPR-Frame as SDC-Frame and decode c-info at receiver*
        ...
        if self.is_receiver:
            if self.in_process and ((self._get_MSSG_in_process()[1]=="SDC:correct_EPR") 
                ...
                raise BadPhaseCallException("pop_sync_SDC_END_PHASE can repeatedly "
                                            "be called only after EPR_PHASE_2 until"
                                            " all stored pairs were retrieved.")
    ...
    def finish_SDC(self, val_C_info=None):
        if self.is_receiver:
            if val_C_info is None:
                ...
                t_fin = time.perf_counter() - self.start_time
                actualize_df = pd.DataFrame([[t_fin, int(val_C_info)]], columns=["t_finish", "valid_C_info"])
                self._actualize_histories(df_to_add=actualize_df, kind="sdc")
        else:
            t_fin = time.perf_counter() - self.start_time
