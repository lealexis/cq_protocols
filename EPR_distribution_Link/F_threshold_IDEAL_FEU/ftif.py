...
from ent_buff_itfc_FT_IDEAL_FEU import EPR_buff_itfc
...
def int2bin(i,length):
    bin_val = [int(j) for j in list(bin(i)[2:].zfill(length))]
    if len(bin_val) > length:
        raise ValueError("Number bigger than allowed passed to int2bin method.")
    return bin_val
...
def sEPR_proto(host: Host, receiver_id , epr_gen: EPR_generator, 
                   ...
                   F_thres=0.5, frm_len=FRAME_LENGTH, verbose_level=0):
    F_ideal_BF_chann = 0
            ...
            if cbit_counter == 1:
                ...
                # EPR-ACK or SDC-ACK
                    ...
                    # SDC-FEEDBACK
                        ...
                        clsicPipeChann.put(0) # send NACK to Bob
                        break
            # receive idealy estimated fidelity
            else:
                F_bits += str(bit)
                if cbit_counter == 10:
                    ...
                    if F_est < F_thres:
                        ...
                        epr_ack = [1, 0]
                        for bit in epr_ack:       
                            clsicPipeChann.put(bit) # Send EPR-ACK 
...
def sSDC_proto(host:Host, receiver_id, qmem_itfc: EPR_buff_itfc, 
    ...
    for mi in range(frm_len):
        ...
        if verbose_level==1:
            print("ALICE/SDC - send SDC-encoded epr half Nr:{q_i}".format(q_i=mi + 1))
            ...
            if cbit_counter == 1:
                ...
                if bit == 1 : 
                    ...
                    # SDC-FEEDBACK
                        ...
                        #for bit in sdc_ack:       
                        clsicPipeChann.put(1) # Send ACK 
                        #time.sleep(INTER_CBIT_TIME)
                        break
                # EPR-NACK
                else:
                    if (verbose_level == 0) or (verbose_level == 1):
                            print("ALICE/SDC - recv EPR-NACK")
                            print("ALICE/SDC - send SDC-ACK")
                    for bit in sdc_ack:       
                        clsicPipeChann.put(bit) # Send SDC-ACK 
                        time.sleep(INTER_CBIT_TIME)
                    break
            else:
                if (cbit_counter < 10) and clsicPipeChann.fst_feedback_in_trans.is_set():
                    continue
                else:
                    if (verbose_level == 0) or (verbose_level == 1):
                            print("ALICE/SDC - send SDC-ACK")
...
def sender_protocol(host:Host, receiver_id, epr_gen: EPR_generator, 
                    ...
                    proto_finished:Event, on_demand:Event, epr_demand_end:Event,
                    finish_simu:Event, F_thres=0.5, frm_len=FRAME_LENGTH, 
                    verbose_level=0):
...
def receiver_protocol(host:Host, qmem_itfc: EPR_buff_itfc, quPipeChann:QuPipe, 
                      ...
                      on_demand:Event, epr_demand_end:Event, F_thres=0.5,
                      frm_len=FRAME_LENGTH, verbose_level=0):
        ...
        try:
            ...
            if count == 1:
                ...
                # Send Type dependent Feedback and wait for last ACK/NACK LEVEL    
                    ...
                    count = 0 # reinitialize count for next QFrame
...
def main():
    ...
    Alice_EPR_gen =  EPR_generator(host=Alice, min_fid=0.7, max_dev=0.05, 
    ...
    DaemonThread(target=receiver_protocol, args=(Bob, qumem_itfc_B, 
                                ...
                                on_demand_epr_finished, F_thres_B, 
                                FRAME_LENGTH, 0)) 

    DaemonThread(target=put_next_process, args=(frame_comm_finished, 0.35))

    DaemonThread(target=sender_protocol, args=(Alice, Bob.host_id, 
                            Alice_EPR_gen, qumem_itfc_A, Qpiped_channel, 
                            Cpiped_channel, rot_error, frame_comm_finished, 
                            on_demand_comm, on_demand_epr_finished, 
                            FINALIZE_simu, F_thres_A, FRAME_LENGTH, 0))
